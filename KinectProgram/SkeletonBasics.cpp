...
int CSkeletonBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	//char* theMessage = (char*) calloc(BUFLEN, sizeof(char));
	// memset(theMessage, 0, BUFLEN);
	/*float ab = 123;
	float cd = 1234;
	float dk = 129;
	Client* client = new Client();
	client->sendTheUDP(ab, cd, dk);*/
...
void CSkeletonBasics::ProcessSkeleton()
{
	...
	//		char theMessage[512] = "";
	//		sprintf_s(theMessage, "%f %f %f", hand_r_x, hand_r_y, hand_r_z);
	//		Client* client = new Client();
	//		client->sendTheUDP(theMessage);
	//	}
	//}	
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        if (NUI_SKELETON_TRACKED == trackingState)
        {
			...
			// OutputDebugStringW(L"My output string.");
			float head_x = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HEAD].x * 1000;
			float head_y = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HEAD].y * 1000;
			float head_z = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HEAD].z * 1000;


			float neck_z = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_SHOULDER_CENTER].z * 1000;

			// OutputDebugStringW(Lhand_r_x);
			// char qual[20] = "/righthand_pos_body";
			//float ab = 123;
			//float cd = 1234;
			//float dk = 129;
			
			float diff = head_z - neck_z;
			Client* client = new Client();
			client->sendTheUDP(head_x, head_y, diff);
			//char theMessage[512] = "";
			//sprintf_s(theMessage, "%f %f %f", hand_r_x, hand_r_y, hand_r_z);
			//char theMessage[512] = "";
			//strcat(theMessage, qual);
			//strcat(theMessage, theCoor);



			// wchar_t szBuff[512];
			// swprintf(szBuff, L"%f %f %f", hand_r_x, hand_r_y, hand_r_z);
